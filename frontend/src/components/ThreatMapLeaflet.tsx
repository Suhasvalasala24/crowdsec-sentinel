...
const ThreatMapLeaflet = () => {
  ...
  // Mock function to convert IP to coordinates (in real app, use IP geolocation service)
  const getCoordinatesFromIP = (ip: string): { lat: number; lng: number } => {
    // Hash the IP to get consistent coordinates
  ...
  useEffect(() => {
    const threatLocations = alerts.slice(0, 20).map((alert, index) => {
